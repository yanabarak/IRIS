<template>
  <div class="projects-general main-block-general col-12 p-0 classic_block" id="projects-general">
    <div class="block-type blue-bg">
      <h6 class="d-flex gap-2 align-items-center sec-color fs-16">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
          fill="currentColor"
          class="bi bi-browser-chrome"
          viewBox="0 0 16 16"
        >
          <path
            fill-rule="evenodd"
            d="M16 8a8 8 0 0 1-7.022 7.94l1.902-7.098a3 3 0 0 0 .05-1.492A3 3 0 0 0 10.237 6h5.511A8 8 0 0 1 16 8M0 8a8 8 0 0 0 7.927 8l1.426-5.321a3 3 0 0 1-.723.255 3 3 0 0 1-1.743-.147 3 3 0 0 1-1.043-.7L.633 4.876A8 8 0 0 0 0 8m5.004-.167L1.108 3.936A8.003 8.003 0 0 1 15.418 5H8.066a3 3 0 0 0-1.252.243 2.99 2.99 0 0 0-1.81 2.59M8 10a2 2 0 1 0 0-4 2 2 0 0 0 0 4"/>
        </svg>
        Browser Extensions
      </h6>
      <div>
        <ul class="neutral-color pt-0">
          <li class="mb-2 pl-2">
            <div class="neutral-color">
              <span class="font-weight-bold sec-color"><router-link :to="{ path: '/UserControl' }"> UserControl </router-link></span> - Blocks unsafe sites
            </div>
            <span class="fs-10 color-500 font-italic"> # (In Development, v1.9, Q3 2025)</span>
          </li>
          <li class="mb-2 pl-2">
            <div><span class="font-weight-bold sec-color">NoteMaster</span> - Webpage notes</div>

            <span class="fs-10 color-500 font-italic"> # (In Development, v1.0.1, Q3 2025)</span>
          </li>
        </ul>
      </div>
    </div>
    <div class="block-type blue-bg classic_block">
      <h6 class="d-flex gap-2 align-items-center sec-color fs-16">
        <b-icon icon="phone" class="icon-style"></b-icon>

        Mobile Apps
      </h6>
      <div>
        <ul class="neutral-color pt-0">
          <li class="mb-2 pl-2">
            <div><span class="font-weight-bold sec-color">DocA</span> - AI document analysis</div>

            <span class="fs-10 color-500 font-italic"> # (In Development, v0.1, Q3 2025)</span>
          </li>
        </ul>
      </div>
    </div>
    <div class="block-type blue-bg classic_block">
      <h6 class="d-flex gap-2 align-items-center sec-color fs-16">
        <b-icon icon="wifi" class="icon-style"></b-icon>

        Web Applications
      </h6>
      <div>
        <ul class="neutral-color pt-0">
          <li class="mb-2 pl-2">
            <div>
              <span class="font-weight-bold sec-color">ChordMaster</span> - Guitar chord
              visualization
            </div>

            <span class="fs-10 color-500 font-italic"> # (In Development, v0.1, Q3 2025)</span>
          </li>
        </ul>
      </div>
    </div>
  </div>
</template>
